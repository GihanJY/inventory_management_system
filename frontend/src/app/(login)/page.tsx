...
export default function Home() {
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) =>  {
    ...
    if (!email || !password) {
      ...
      const responce = await axios.post(`${baseUrl}/users/loginUser`, {email, password});
      ...
      <div className="max-w-screen-xl m-0 sm:m-10 bg-white dark:bg-gray-800 shadow sm:rounded-lg flex justify-center flex-1">
        <div className="lg:w-1/2 xl:w-5/12 p-6 sm:p-12">
          <div className="flex flex-row items-center justify-center text-center gap-3">
            ...
            <p className="text-indigo-500 dark:text-indigo-400 font-bold text-2xl">StockManage</p>
          </div>
  ...
  <button onClick={() => setTheme(theme === "light" ? "dark" : "light")} className="mr-2">
    {theme === "light" ? <Sun size={20} /> : <MoonStarIcon size={20} />}
  </button>
  <p className="text-xs text-gray-600 dark:text-gray-400 text-center">
    I agree to abide by templatana's
    <a href="#" className="border-b border-gray-500 dark:border-gray-400 border-dotted">
      Terms of Service
    </a>
    and its
    <a href="#" className="border-b border-gray-500 dark:border-gray-400 border-dotted">
